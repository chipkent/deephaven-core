...
public class TailInitializationFilter {
    ...
    public static Table mostRecent(final Table table, final String timestampName, final long nanos) {
        return QueryPerformanceRecorder.withNugget("TailInitializationFilter(" + nanos + ")", () -> {
            final ColumnSource timestampSource = table.getColumnSource(timestampName, Instant.class);
    ...
    private static Table mostRecentDateTime(final Table table, final ColumnSource<Instant> cs, final long nanos) {
        return mostRecentLong(table, (idx) -> DateTimeUtils.epochNanos(cs.get(idx)), nanos);
    }
