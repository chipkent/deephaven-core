    ...
    BucketingContext(final String listenerPrefix, final QueryTable leftTable, final QueryTable rightTable,
        ...
        for (int ii = 0; ii < keyColumnCount; ++ii) {
            ...
            if (leftType == Instant.class) {
                ...
                leftSources[ii] = ReinterpretUtils.dateTimeToLongSource((ColumnSource<Instant>) leftSources[ii]);
                // noinspection unchecked
                rightSources[ii] = ReinterpretUtils.dateTimeToLongSource((ColumnSource<Instant>) rightSources[ii]);
