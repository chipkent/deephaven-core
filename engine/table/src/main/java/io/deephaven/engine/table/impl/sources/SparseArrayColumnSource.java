...
public abstract class SparseArrayColumnSource<T>
    ...
    public static WritableColumnSource<Instant> getDateTimeMemoryColumnSource(long[] data) {
        final WritableColumnSource<Instant> result = new DateTimeSparseArraySource();
        result.ensureCapacity(data.length);
        long i = 0;
        for (long o : data) {
            result.set(i++, o);
