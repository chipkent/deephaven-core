...
public class TestReinterpretedColumn extends RefreshingTableTestCase {
    ...
    private final Instant baseDateTime = DateTimeUtils.parseInstant("2021-10-19T10:30:00.000 NY");
    ...
    private QueryTable makeObjectTable(WritableColumnSource<Long> longSource, WritableColumnSource<Instant> dtSource,
        ...
        for (int ii = 0; ii < ROW_COUNT; ii++) {
            ...
            dtSource.set(ii, DateTimeUtils.epochNanosToInstant(DateTimeUtils.epochNanos(baseDateTime) + tOff));
    ...
    private QueryTable makeTable(WritableColumnSource<Long> longSource, WritableColumnSource<Instant> dtSource,
            WritableColumnSource<Instant> iSource, WritableColumnSource<ZonedDateTime> zdtSource) {
        longSource.ensureCapacity(ROW_COUNT);
        dtSource.ensureCapacity(ROW_COUNT);
        iSource.ensureCapacity(ROW_COUNT);
        ...
        for (int ii = 0; ii < ROW_COUNT; ii++) {
            ...
            dtSource.set(ii, DateTimeUtils.epochNanos(baseDateTime) + tOff);
    ...
    private void testReinterpretLong(final Table initial, boolean isSorted, boolean withRename) {
        ...
        for (final RowSet.Iterator it = table.getRowSet().iterator(); it.hasNext();) {
            ...
            assertEquals(DateTimeUtils.epochNanos(baseDateTime) + tOff, table.getColumnSource(dtColName).getLong(key));
        ...
        reinterpLongChunkCheck(table.getColumnSource(dtColName), table.getRowSet(), isSorted,
                DateTimeUtils.epochNanos(baseDateTime));
    ...
    private <T> void doReinterpretTestBasic(final Table initial,
        ...
        for (final RowSet.Iterator it = table.getRowSet().iterator(); it.hasNext();) {
            ...
            assertEquals(DateTimeUtils.epochNanos(baseDateTime) + tOff,
        ...
        reinterpBasicChunkCheck(table.getColumnSource(dtColName), table.getRowSet(), toNanoFunc, isSorted,
                DateTimeUtils.epochNanos(baseDateTime), extraCheck);
    ...
    public void testReinterpretDBDT() {
        doReinterpretTestBasic(
                baseTable, Instant.class, TableTimeConversions::asDateTime, "DT", DateTimeUtils::epochNanos);
        doReinterpretTestBasic(
                sparseBaseTable, Instant.class, TableTimeConversions::asDateTime, "DT", DateTimeUtils::epochNanos);
        doReinterpretTestBasic(
                objectTable, Instant.class, TableTimeConversions::asDateTime, "DT", DateTimeUtils::epochNanos);
        doReinterpretTestBasic(
                sparseObjectTable, Instant.class, TableTimeConversions::asDateTime, "DT", DateTimeUtils::epochNanos);
    }
